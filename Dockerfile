...
# Download URL template from futunn.com official docs
...
RUN curl -sL -o opend.tar.gz "${OPEND_URL}" \
    && mkdir -p opend \
    && tar xzf opend.tar.gz -C opend --strip-components=1 \
    && ls -la opend/
...
# Copy OpenD binaries from downloader stage
...
# This solves the "cross-network communication need to be encrypted" error
...
# Expose ports
# 11111 - API protocol (行情+交易)
# 22222 - Telnet remote commands
EXPOSE 11111
EXPOSE 22222

# Health check: verify OpenD port is listening
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
    CMD bash -c 'echo > /dev/tcp/localhost/11111' || exit 1

# Default: run with config volume mount
# Mount your FutuOpenD.xml to /app/config/FutuOpenD.xml
# Example: docker run -v ./config:/app/config futu-opend
#
# Or use environment variables to override login credentials:
#   -e FUTU_LOGIN_ACCOUNT=100000
#   -e FUTU_LOGIN_PWD=your_password
#
# Or use command-line parameters directly:
#   docker run futu-opend -login_account=100000 -login_pwd=xxx

# If config file exists, use it; otherwise fall back to env vars or CLI args
